...
namespace SocialPlay_Editor
{
    ...
    public class AtlasGenerationWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            GUILayout.Space(40);
            GUILayout.BeginHorizontal();
            atlasName = EditorGUILayout.TextField(new GUIContent("Pick atlas name:", "Changing the name will re-download all the images from the server"), atlasName, GUILayout.Width(Screen.width / 2));
            ...
            if (GUILayout.Button(new GUIContent("Discard Current", "Disacards the data for the atlas with the selected name"))) { Discard(); }
